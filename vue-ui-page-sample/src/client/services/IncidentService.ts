...
import type { Incident, IncidentPayload } from '../types'

export class IncidentService {
    private readonly tableName: string

    constructor() {
        this.tableName = 'x_vueuisample_incident'
    }
    ...
    async get(sysId: string): Promise<Incident> {
        try {
            const searchParams = new URLSearchParams()
            searchParams.set('sysparm_display_value', 'all')
    ...
    async create(data: IncidentPayload): Promise<any> {
        try {
    ...
    // Update an incident
    async update(sysId: string, data: IncidentPayload): Promise<any> {
        try {
